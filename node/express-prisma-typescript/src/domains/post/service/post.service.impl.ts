...
export class PostServiceImpl implements PostService {
  ...
  async getLatestPosts(userId: string, options: CursorPagination): Promise<ExtendedPostDTO[]> {
    ...
    const posts = await this.postRepository.getAllByDatePaginated(userId,usersFilter, options);
    if (posts) {
  ...
  async getFollowPosts(
    ...
    const usersFilter = await this.followRepository.getFollowedIds(userId);
    if (!userId) throw new NotFoundException('user');
    const posts = await this.postRepository.getFollowPostsPaginated(userId, usersFilter, options);
    if (posts) {
      const postWithUrl = await this.getUrlsArray(posts);
      return postWithUrl;
    } else {
